...
public:
	...
	bool check_if_recieved_data(){return strlen((const char*)read_buffer) > 0;};
	void send_data_to_buffer(uint8_t* data, size_t data_length);
	void read_data_from_buffer(uint8_t* data, size_t data_length);
	uint8_t* get_read_buffer(){return read_buffer;};
